...
Each language has its own context, which it uses to manage its pools, among other things. Moving between passes for the same language needs no further memory allocation resource-wise: the types, symbols, nodes and everything else moves to the next pass as its input. The new pass is able to read them, decorate them, reorder subtrees, introduce new types via type descriptors, etc.

When moving between languages, however, some pools become meaningless: there's no use for the Julia AST when the current pass is already operating in GLSL-land, its input completely disconnected from any Julia resources. The types, qualifiers, symbols and source locations on the other hand, can all be moved between language contexts without any extra hassle.
